...
#define VMS_CODE_FLASH_INTERMEDIATE_BASE_ADDRESS  0x9d008000

/* Virtual address for VM's code when it resides on RAM. */
#define VMS_CODE_FLASH_VIRTUAL_BASE_ADDRESS  0x9d000000

/* Intermediate Physical address of the first VM on the FLASH */
#define VMS_CODE_RAM_INTERMEDIATE_BASE_ADDRESS  0x80010000

/* Virtual address for VM's FLASH */
#define VMS_CODE_RAM_VIRTUAL_BASE_ADDRESS  0x80000000
 ...
 * @return 0 if sucessfull or EXIT_FAILURE in case of error. 
 */
int gen_conf_vms(config_t cfg, FILE* outfile, char *app_list, int* vm_count, char* vms_info, enum platform_t platform){
	int vm_number = 1;
	unsigned int vm_ram_inter_addr = VMS_RAM_INTERMEDIATE_BASE_ADDRESS;
	unsigned int vm_code_inter_addr;
	int i, num_el, ret, aux, ram_size, code_size = 0, j, num_mm;
	unsigned int value;
	...
	unsigned int priority = 0;
        ...
        if(platform == PIC32MZ){
            vm_code_inter_addr = VMS_CODE_FLASH_INTERMEDIATE_BASE_ADDRESS;
        }else{
            vm_code_inter_addr = VMS_CODE_RAM_INTERMEDIATE_BASE_ADDRESS;
        }
		...
		/* get FLASH size */
                if (platform == PIC32MZ){
                    if( !config_setting_lookup_string(vm_conf, "flash_size_bytes", &auxstrp)){
                        ...
                        if ( (code_size = get_value_from_str((struct mem_sizes_def*)MemSizes, sizeof(MemSizes)/sizeof(struct mem_sizes_def), (char*)auxstrp)) == 0){
                                fprintf(stderr, "Invalide value for flash_size_bytes: %s.\n", auxstrp);
                                return EXIT_FAILURE;
                        }
                        /* Create a TLB entry to the FLASH memory */
                        if ((ret = process_tlb_entry(vm_number, code_size, &vm_code_inter_addr, VMS_CODE_FLASH_VIRTUAL_BASE_ADDRESS, "WRITE_BACK", outfile))){
                                return ret;
                        }
                    }
                }
...
int main(int argc, char **argv)
{
	...
	char* vms_info;
	struct stat st = {0};
        enum platform_t platform = PIC32MZ;
    
	if (argc<2){
		fprintf(stderr, "Usage: %s <config file path> [platform]\nplatform: pic32mz or baikal-t1", argv[0]);
		return EXIT_FAILURE;
	}
	...
	if(argc>=3){
            if(!strcmp(argv[2], "pic32mz")){
                platform = PIC32MZ;
            }else if (!strcmp(argv[2], "baikal-t1")){
                platform = BAIKAL_T1;
            }
        }
	...
	if (gen_conf_vms(cfg, outfile, app_list, &vm_count, vms_info, platform)){
		config_destroy(&cfg);
		fclose(outfile);
		return(EXIT_FAILURE);
	}
